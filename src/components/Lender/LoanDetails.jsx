...
//             <div className="space-y-4">
//               {loan.emiPaidCount < loan.repaymentPeriod ? ( <>Active</>
//               ) : (
...
//           {userAddress === loan.lender && (
//             <div className="space-y-4">
//               {!loan.status.inDefault && (
//                 <button
//                   className="bg-yellow-500 text-white px-6 py-2 rounded-lg hover:bg-yellow-600 w-full transition-colors"
//                   onClick={() => handleTransaction('checkDefault')}
//                 >
...
//               {loan.status.inDefault && (
//                 <button
//                   className={`bg-red-500 text-white px-6 py-2 rounded-lg w-full transition-colors ${
//                     timeRemaining?.includes('available now') 
//                       ? 'hover:bg-red-600' 
//                       : 'opacity-50 cursor-not-allowed'
//                   }`}
//                   onClick={() => handleTransaction('liquidate')}
//                   disabled={!timeRemaining?.includes('available now')}
//                 >
//                   {timeRemaining || 'Liquidate Collateral'}
//                 </button>
//               )}
//             </div>
//           )}
...
const LoanDetails = () => {
  ...
  if (!loan) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-gradient-to-r from-gray-100 to-gray-300">
        <p className="text-xl text-gray-700 mb-4">Loading loan details...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-200 p-6 flex flex-col items-center">
      <div className="max-w-2xl w-full bg-white shadow-xl rounded-xl p-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">Loan Details</h1>
        
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <p>
              <span className="font-semibold text-gray-700">Loan ID:</span> 
              <span className="text-gray-900 break-all ml-2">{loan.id}</span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">Farmer:</span> 
              <span className="text-gray-900 break-all ml-2">{loan.farmer}</span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">Lender:</span> 
              <span className="text-gray-900 break-all ml-2">
                {loan.lender || "Not assigned"}
              </span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">Amount:</span> 
              <span className="text-gray-900 ml-2">{loan.amount} ETH</span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">Repayment Period:</span> 
              <span className="text-gray-900 ml-2">{loan.repaymentPeriod} months</span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">EMI Amount:</span> 
              <span className="text-gray-900 ml-2">{parseInt(loan.emi)/1e18} ETH</span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">EMIs Paid:</span> 
            ...
            <p>
              ...
              <span className="text-gray-900 ml-2">
                {loan.emiPaidCount ? loan.lastPaymentDate.toLocaleDateString() : "Not yet started"}
              </span>
            </p>
            <p>
              <span className="font-semibold text-gray-700">NFT Collateral:</span> 
              <span className="text-gray-900 break-all ml-2 block">
                Token ID: {loan.tokenId} <br/>
                Contract: {loan.nftContract}
              </span>
            </p>
          ...
          <div className="p-6 rounded-xl bg-gray-50 border border-gray-200">
            <div className="grid grid-cols-2 gap-4">
              <p>
                <span className="font-semibold text-gray-700">Sanctioned:</span> 
                <span className={loan.status.sanctioned ? "text-green-600 ml-2" : "text-red-600 ml-2"}>
                  {loan.status.sanctioned ? "Yes" : "No"}
                </span>
              </p>
              <p>
                <span className="font-semibold text-gray-700">Approved:</span> 
                <span className={loan.status.approved ? "text-green-600 ml-2" : "text-red-600 ml-2"}>
                  {loan.status.approved ? "Yes" : "No"}
                </span>
              </p>
              <p>
                <span className="font-semibold text-gray-700">Rejected:</span> 
                <span className={loan.status.rejected ? "text-red-600 ml-2" : "text-gray-600 ml-2"}>
                  {loan.status.rejected ? "Yes" : "No"}
                </span>
              </p>
              <p>
                <span className="font-semibold text-gray-700">In Default:</span> 
                <span className={loan.status.inDefault ? "text-red-600 ml-2" : "text-green-600 ml-2"}>
                  {loan.status.inDefault ? "Yes" : "No"}
                </span>
              </p>
            </div>
          ...
          <div className="mt-2">
            <p>
              <span className="font-semibold text-gray-700">Document CID:</span> 
              <span className="text-gray-900 break-all ml-2">{loan.cid}</span>
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-col gap-4">
          <button
            onClick={() => navigate("/myloans")}
            className="bg-blue-600 text-white px-8 py-3 rounded-full shadow hover:bg-blue-700 transition duration-200"
          >
            Back to Loans
          </button>
          ...
          {loan.status.sanctioned && !loan.status.inDefault && (
            <div className="text-center">
              {loan.emiPaidCount < loan.repaymentPeriod ? (
                <span className="text-lg text-indigo-600 font-medium">Active</span>
              ) : (
          ...
          {userAddress === loan.lender && (
            <div className="flex flex-col gap-4">
              {!loan.status.inDefault && (
                <button
                  className="bg-yellow-500 text-white px-8 py-3 rounded-full shadow hover:bg-yellow-600 transition duration-200"
                  onClick={() => handleTransaction('checkDefault')}
                >
                  Check Default Status
                </button>
              ...
              {loan.status.inDefault && (
                <button
                  className={`bg-red-500 text-white px-8 py-3 rounded-full shadow transition duration-200 ${
                    timeRemaining?.includes('available now') 
                      ? 'hover:bg-red-600' 
                      : 'opacity-50 cursor-not-allowed'
                  }`}
